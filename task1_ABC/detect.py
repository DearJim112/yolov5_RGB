            ...
            if len(det):
                ...
                with open(txt_path + '.txt', 'w') as f:
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4)) / gn).view(-1).tolist()  # normalized xywh  直接显示像素就不用计算了
                        line = (cls, *xywh, conf) if save_conf else (cls, *xywh)  # label format
                    # line = (cls, *xyxy, conf) if save_conf else (cls, *xyxy)  # 直接存放像素
                    # with open(txt_path + '.txt', 'w') as f:
...
def parse_opt():
    ...
    # parser.add_argument('--source', type=str, default=ROOT / 'RGYlightpic/images/test', help='file/dir/URL/glob, 0 for webcam')
    parser.add_argument('--source', type=str, default=ROOT / './task1', help='file/dir/URL/glob, 0 for webcam')
    ...
    parser.add_argument('--max-det', type=int, default=15, help='maximum detections per image')
...
if __name__ == "__main__":
    ...
    print('输入任何字符，开始识别task1下的所有图像')
    input()
    opt = parse_opt()
    main(opt)

    addjsonpath = increment_path(jsonpath, exist_ok=True)  # increment run
    addjsonpath.mkdir(parents=True, exist_ok=True)  # make dir
    txt2json(txtpath, jsonpath, imagepath)
    print('json存放地址：',jsonpath)